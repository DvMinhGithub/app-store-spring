...
public class UserController {
    ...
    public ApiResponse<UserResponse> findByEmailOrPhone(
            ...
            @RequestParam("phone") @Nullable @Pattern(regexp = "\\d{10}", message = "Phone must be 10 digits")
                    String phone) {

        if ((email == null || email.isEmpty()) && (phone == null || phone.isEmpty())) {
            throw new IllegalArgumentException("At least one of email or phone must be provided");
        }
        return ApiResponse.success(userService.findByEmailOrPhone(email, phone), "User fetched successfully");
    }

    @PutMapping("/{id}")
    public ApiResponse<Void> updateUser(@PathVariable("id") Long id, @RequestBody @Valid UserUpdateRequest user) {
        userService.updateUser(id, user);
        return ApiResponse.success("User updated successfully");
    }

    @PostMapping("/{id}/roles")
    public ApiResponse<Void> assignRoles(@PathVariable("id") Long userId, @RequestBody List<Long> roleIds) {
